...
- The check-mode ASSERTION side (remap without INVTP must fire the
  phantom-cache CHECKFAIL, ptbase change without asid/INVTP likewise)
  cannot live in a passing run: it ends exit 3 with an implementation-
  defined reason line. Needs a harness extension (expected-CHECKFAIL
  manifest class) — iteration 3, noted in tests/README.md. The legal
  sides (INVTP'd remap, asid'd switch) are asserted here, under
  --check-invtp, which run-tests always passes.
...
def main():
    emit("# c2_mmu — GENERATED by tests/gen_c2.py — DO NOT EDIT.")
    emit("# CONFORMANCE.md group C2: MMU runtime remap. Assembled after")
    emit("# tests/defs.s. Layout rationale + bounded-coverage notes in")
    emit("# gen_c2.py's docstring.")
    emit()
    for name, v in [
    ...
    # ROOT2 maps VPN0 to the SAME frame as ROOT does: the two fetches
    # between the ptbase and asid writes in test [32] then translate
    # identically under either table, so the switch window cannot trip
    # a result-comparing stale check (SPEC-ISSUES 21).
    node(ROOT2, 0, 0, mask_above(0), {
        0: leaf(0x00000, "R", "W", "X"),
        1: leaf(0x100000, "R", "W"),
    })
