...
- atomicity under interrupts (timer armed to fire "during" an AMO)
  needs the C1 timer/handler scaffolding; lands with C1 in a later
  iteration, verified from the trace (no event between the access pair)
- AMO/CAS to device space (DEVERR) and atomic permission-fault ordering
  (load-before-store) are C7/C2-adjacent and need device space / MMU
  scaffolding; same later iteration
- atomic UNALIGNED traps (covered structurally with C7's alignment
  sweep)
"""
...
def generate():
    ...
    emit("# Bounded coverage: interrupt-atomicity, device DEVERR, and")
    emit("# permission-order cases land with C1/C2/C7 scaffolding — see")
    emit("# the generator header. Conventions per tests/README.md.")
    emit()
    emit("        .org 0x1000")
    emit("start:")
